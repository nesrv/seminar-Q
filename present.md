...
В 2024 году ИИ умеет не только отвечать на вопросы и рисовать картинки, но и писать код.

Получается пока с переменным успехом. 
...
Задачи из курсов по введению в `SQL` первый попавшийся сайт с GPT⁠-⁠3.5 на борту решил с первого раза за секунды. 

Искать ответы через `Google` оказалось дольше
...
Или попросить «понюхать» выбранный кусок кода — и в ответ получите список, куда войдут не только необходимые фиксы, но и пункты «на подумать». 

Например, большое количество if в коде; если это вам не нравится, можно попросить оптимизировать код. 
...
`SourceGraph` — это платный продукт, но `Cody AI` — бесплатный; сейчас, на ранней стадии продукта, единственное ограничение — это количество запросов к `GPT 3.5`, 250 в месяц. 

`Cody AI` экономичен в запросах и умело сжимает контекст, поэтому лимит получается не такой уж и маленький.
...
Но при этом бесплатный. 

`CodeWhisperer` активно развивается, и если вы первый раз хотите попробовать AI-инструменты в работе, то это будет хорошим стартом, прежде чем вы решитесь на еще одну строчку расходов.
...
* Генерирует любой код, начиная с небольших фрагментов и заканчивая полноценными функциями с помощью искусственного интеллекта.

* Фильтрует предложения кода, которые могут считаться предвзятыми или необъективными.

* Фильтрует или выделяет код, похожий на обучающие данные с открытым исходным кодом.

* Сканирует код на наличие уязвимостей безопасности.
...
[https://ru.hexlet.io/blog/posts/amazon-codewhisperer](https://ru.hexlet.io/blog/posts/amazon-codewhisperer)